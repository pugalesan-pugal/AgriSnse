  ...
  async function handleForgotNext(e: React.FormEvent) {
    ...
    try {
      ...
      if (forgotStep === 1) {
        ...
        if (passwordStrength(fpNew) < 3) throw new Error("Password too weak");
    ...
    <main className="container-responsive py-12 md:py-20 grid lg:grid-cols-2 gap-10 items-center min-h-[80vh]">
      <section className="order-2 lg:order-1">
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl p-6 md:p-8 bg-white/90 shadow-md border border-emerald-900/5 max-w-md animate-fade-up">
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full mt-2 transition-transform duration-200 hover:scale-[1.01]">{loading ? "Signing in..." : "Sign In"}</button>
              ...
              {forgotStep === 2 && (
                <>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button type="button" disabled={fpLoading} onClick={async ()=>{ setError(null); setFpInfo(null); setFpLoading(true); try { const s = await fetch("/api/auth/send-otp", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: fpEmail }) }); const sd = await s.json(); if (!s.ok) throw new Error(sd.error || "Failed"); setFpInfo(`OTP re-sent to ${fpEmail}`); } catch (e:any) { setError(e.message || "Failed"); } finally { setFpLoading(false); } }} className="chip">Resend</button>
                  </div>
                </>
              )}
              {forgotStep === 3 && (
                <>
                  ...
                  <div className="text-xs mb-2" style={{ color: "#275539" }}>Strength: {passwordStrength(fpNew)} / 5</div>
